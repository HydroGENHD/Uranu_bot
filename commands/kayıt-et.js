...
if (!kayıtlı) return interaction.reply("<a:carpii:1142783868089417819> Kayıtlı rolü ayarlanmamış!")
interaction.guild.members.cache.get(user.id).roles.add(kayıtlı)
user.setNickname(`${isim} | ${yas}`)

    interaction.reply({content: `<a:onay:1125386573576425522> Başarıyla ${user} Kullanıcısını Kayıt Ettim!`})
}
