...
client.on("guildMemberAdd", member => {
  ...
  if(!kanal) return;
  member.guild.channels.cache.get(kanal).send({content: `<:katildi:1149691243064987839> | ${member} sunucuya katıldı! Sunucumuz **${member.guild.memberCount}** kişi oldu.`})
})
...
client.on("messageCreate", async message => {
  ...
  if (await db.get(`afk_${message.author.id}`)) {
    ...
    message.reply("<a:onay:1125386573576425522> Afk Modundan Başarıyla Çıkış Yaptın!");
  }
  ...
  if (sebep) {
    message.reply("<:carpi:1149699793287925810> Etiketlediğin Kullanıcı **"+sebep+"** Sebebiyle Afk Modunda!");
  }
});
client.on("guildMemberAdd", member => {
...
client.on("guildMemberRemove", member => {
  const kanal = db.get(`hgbb_${member.guild.id}`)
  if(!kanal) return;
  member.guild.channels.cache.get(kanal).send({content: `<:ayrildi:1149701235239301130> | ${member} sunucudan ayrıldı! Sunucumuz **${member.guild.memberCount}** kişi oldu.`})
})
...
message.delete()
message.channel.send(`<a:uyari2:1142546781733912606> Hey <@${message.author.id}>, Bu Sunucuda Küfür Engel Sistemi Aktif! `)
}
}
})
client.on("messageCreate", (message) => {
...
message.delete()
message.channel.send(`<a:uyari2:1142546781733912606> Hey <@${message.author.id}>, Bu Sunucuda Reklam Engel Sistemi Aktif! `)
}
}
})
...
message.channel.send(`<@${message.author.id}> Aleyküm Selam, Hoşgeldin ! `)
}
}
})
